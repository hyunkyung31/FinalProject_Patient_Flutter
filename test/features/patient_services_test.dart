...
class ServiceAdapter implements HttpClientAdapter {
  ...
  Future<ResponseBody> fetch(
    ...
    final body = queuedResults.isEmpty ? result : queuedResults.removeAt(0);
...
void main() {
  testWidgets('실제 앱의 다크 테마가 어두운 배경에 밝은 글씨를 적용한다', (tester) async {
    ...
    await tester.pumpWidget(
      MyApp(onboardingRepository: IncompleteOnboarding()),
    );
    await tester.pump();
  ...
  test('동의 문서 페이지와 다음 페이지 여부를 파싱한다', () async {
    adapter.result = {
      ...
      'next': 'https://example.invalid/?page=3',
    };
  ...
  test('활성 필수 문서 중 동의하지 않은 문서만 첫 로그인 동의 대상으로 반환한다', () async {
    adapter.queuedResults.addAll([
      {
        'results': [
          {
            'id': 1,
      ...
      },
      {
        'results': [
          {
            'status': 'CONSENTED',
            'consent_document': {'id': 1},
          },
          {
            'status': 'WITHDRAWN',
            'consent_document': {'id': 4},
          },
        ],
        'next': null,
      },
    ]);
    final documents = await repository.requiredConsentDocuments();
    expect(documents.map((document) => document['id']), [2]);
  ...
  testWidgets('환자정보 화면은 서버 값을 표시한다', (tester) async {
    adapter.result = {
      ...
      'gender': 'FEMALE',
      'contact': '01012345678',
      'medical_record_no': 'TEST-001',
    };
    await tester.pumpWidget(
      MaterialApp(
        home: PatientServicesScreen(
